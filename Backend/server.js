...
// Dynamically handle extra production domains from env if specified
if (process.env.FRONTEND_RENDER_URL) {
  // Ensure we strip any accidental trailing slash from the env variable
  const cleanedUrl = process.env.FRONTEND_RENDER_URL.replace(/\/$/, "");
  allowedOrigins.push(cleanedUrl);
}

// Pass the dynamic validation function using the allowedOrigins array
app.use(cors({ 
  origin: function (origin, callback) {
    // Allow requests with no origin (like mobile apps, postman, curl, or server-to-server)
    if (!origin) return callback(null, true);
    
    // Strip trailing slash from coming origin just in case
